...
from PyQt5.QtWidgets import QWidget, QMainWindow, QPushButton, QLineEdit, QLabel, QVBoxLayout, QHBoxLayout, QFrame, \
...
class HomeScreen(QMainWindow):
    def __init__(self):
        ...
        self.btn_home.setStyleSheet(
            "width:200px;height:160px;font-size:20px;background:#1A3668;color:#fff;font-weight:bold;border:1px solid white")
        self.btn_add.setStyleSheet(
            "width:200px;height:160px;font-size:20px;background:#A4866F;color:#fff;font-weight:bold;border:1px solid white")
        ...
        self.timer.timeout.connect(self.refreshPages)
    ...
    def addAddStudentPage(self):
        ...
        mobile_label = QLabel("Mobile(+91XXXXXXXXXX): ")
        ...
        button.clicked.connect(
            lambda: self.addVehicles(name_input.text(), vehicle_input.text(), mobile_input.text(), vtype.currentIndex(),
                                     error_label))
    ...
    def addVehicles(self, name, vehicleno, mobile, index, error_label):
        if not re.match(r'^\+91\d{10}$', mobile):
            error_label.setText("Invalid mobile number format")
            return
        vtp = 2
        if index == 0:
            vtp = 2
        ...
        message = f"Dear {name}, your vehicle with number {vehicleno} has been added to the parking system."
        try:
    ...
    # def send_sms(self, name, vehicleno, mobile):
